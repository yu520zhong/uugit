...
(function($){
	...
	window.wxc.xcConfirm = function(popHtml, type, options) {
		...
		var $popTip = $("<div>").addClass("popTip");//弹窗盒子
		var $ttBox = $("<div>").addClass("ttBox");//弹窗顶部区域
		var $txtBox = $("<div>").addClass("txtBox");//弹窗内容主体区
		var $btnArea = $("<div>").addClass("btnArea");//按钮区域

		var $ok = $("<a>").addClass("sgBtn").addClass("ok").text("确 定");//确定按钮
		var $cancel = $("<a>").addClass("sgBtn").addClass("cancel").text("取 消");
		var $look = $("<a>").addClass("sgBtn").addClass("ok").text("查看");
		...
		function creatDom(){
			...
			$box.attr("id", popId).append($layer).append($popTip);
			$("body").append($box);
		}
